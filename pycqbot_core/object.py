...
from pycqbot_core.cqCode import reply, strToCqCode, get_cq_code, node_list

if TYPE_CHECKING:
    from pycqbot_core import cqBot, cqHttpApi
...
class Message:
    ...
    def __init__(self, message_data: dict[str, Any], cqapi: cqHttpApi):
        ...
        self.code: list[dict[str, Union[str, dict[str, Any]]]] = [
    ...
    def reply(self, message: str, auto_escape: bool = False):
        """
        回复该消息
        """
        return self._cqapi.send_reply(self, "%s%s" % (
            reply(msg_id=self.id), message), auto_escape)

    def reply_not_code(self, message: str, auto_escape: bool = False):
        """
        回复该消息 不带 cqcode
        """
        return self._cqapi.send_reply(self, message, auto_escape)
    ...
    def sent_forward_message(self, message: list):

        if self.type == "group":
            if self.group_id is None:
                return

            return self._cqapi.send_group_forward_msg(
                self.group_id,
                node_list(
                    message, "小心海", 3265595625)
            )

        if self.type == "private":
            return self._cqapi.send_private_forward_msg(
                self.user_id,
                node_list(
                    message, "小心海", 3265595625)
            )
...
class cqEvent:
    ...
    def notice_group_upload(self, message: Message):
        """
        群文件上传
        """
        pass
