...
class cqSocket:
    ...
    def start(self, go_cqhttp_path: str = "./", print_error: bool = True, start_go_cqhttp: bool = True) -> None:
        ...
        def cqhttp_start():
            ...
            if plat == 'windows':
                subp = subprocess.Popen("cd %s && .\go-cqhttp.exe -faststart" %
                                        go_cqhttp_path, shell=True, stdout=subprocess.PIPE)
            elif plat == 'linux':
                subp = subprocess.Popen("cd %s && ./go-cqhttp -faststart" %
                                        go_cqhttp_path, shell=True, stdout=subprocess.PIPE)
    ...
    def _websocket_start(self) -> None:
        ...
        async def main_logic():
            while self.reconnection != -1:
                ...
                self.reconnection -= 1
                logging.warning(
                    f"{self.reconnection_sleep}秒后 重新连接 websocket 服务 ({old_reconnection - self.reconnection}/{old_reconnection})")
...
class asyncHttp:
    ...
    def add_task(self, coroutine: Coroutine) -> None:
        logging.info("add_task,run coroutine")
        asyncio.run_coroutine_threadsafe(coroutine, self._loop)

    def add(self, api: str, data: dict = None):
        if data is None:
            data = {}

        return asyncio.run_coroutine_threadsafe(
            self._asynclink(api, data), self._loop).result()
