...
def commandDataRemoveAllCqcode(commandData: list) -> list:
    i = 0
    while i < len(commandData):

        cqCodeStrList = strToCqCode(commandData[i])

        if bool(cqCodeStrList):
            commandData.remove(commandData[i])
        else:
            i += 1
...
def strRemoveAllCqCode(message: str) -> str:
    cqCodeList = strToCqCode(message)

    for cqCode in cqCodeList:
        message.replace(cqCode, '')
