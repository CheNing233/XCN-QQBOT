...
import time
import sqlite3
from pycqbot_core.object import Message, cqEvent
from pycqbot_core.socketApp import cqSocket, asyncHttp
...
class cqHttpApi(asyncHttp):
    ...
    def _create_sql_link(self, db_path: str, sleep: int) -> None:
        ...
        thread = Thread(target=self._record_message_ck,
                        args=(sleep,), name="_record_message_ck")
    ...
    def _record_message_ck(self, sleep: int) -> None:
        ...
        while True:
            try:
                with sqlite3.connect(self._db_path) as sql_link:
                    sql_cursor = sql_link.cursor()
                    data_list = sql_cursor.execute(
                        "SELECT * FROM `Message` WHERE etime < '%s'" % int(time.time()))
                    for data in data_list:
                        sql_cursor.execute(
                            "DELETE from `Message` where ID = '%s'" % data[0])
        ...
        try:
            with sqlite3.connect(self._db_path) as sql_link:
                ...
                data_cursor = sql_cursor.execute(
                    "SELECT * FROM `Message` WHERE userId = '%s'" % user_id)
    ...
    def send_private_msg(self, user_id: int, message: str, group_id: Union[int, str] = "", auto_escape: bool = False):
        """
        发送私聊消息
        """
        post_data = {
        ...
        return self.add("/send_msg", post_data)

    def send_group_msg(self, group_id: Union[int, str], message: str, auto_escape: bool = False):
        """
        发送群消息
        """
        post_data = {
    ...
    def send_group_forward_msg(self, group_id: int, message: str):
        """
        发送合并转发 ( 群 )
        """
        post_data = {
        ...
        return self.add("/send_group_forward_msg", post_data)

    def send_private_forward_msg(self, user_id: int, message: str):
        """
        发送合并转发 ( 私聊 )
        """
        post_data = {
        ...
        return self.add("/send_private_forward_msg", post_data)

    def send_reply(self, from_message: Message, message: str, auto_escape: bool = False):
        """
        发送回复
        """
        if from_message.type == "group":
            ...
            return self.send_group_msg(from_message.group_id, message, auto_escape)

        if from_message.type == "private":
            return self.send_private_msg(from_message.user_id, message, auto_escape)
    ...
    def set_group_ban(self, group_id: int, user_id: int, duration: int = 30):
        ...
        return self.add("/set_group_ban", post_data)
    ...
    def set_group_whole_ban(self, group_id: int, enable: bool = True):
        ...
        return self.add("/set_group_whole_ban", post_data)
    ...
    def set_group_admin(self, group_id: int, user_id: int, enable: bool = True):
        ...
        return self.add("/set_group_admin", post_data)
    ...
    def set_group_card(self, group_id: int, user_id: int, card: str = ""):
        ...
        return self.add("/set_group_card", post_data)
    ...
    def set_group_name(self, group_id: int, group_name: str):
        ...
        return self.add("/set_group_name", post_data)
    ...
    def set_group_leave(self, group_id: int, is_dismiss: bool = False):
        ...
        return self.add("/set_group_leave", post_data)
    ...
    def set_group_special_title(self, group_id, user_id, special_title="", duration=-1):
        ...
        return self.add("/set_group_special_title", post_data)
    ...
    def set_friend_add_request(self, flag, approve, remark):
        ...
        return self.add("/set_friend_add_request", post_data)
    ...
    def set_group_add_request(self, flag, sub_type, approve=True, reason=""):
        ...
        return self.add("/set_group_add_request", post_data)
    ...
    def get_group_file_url(self, group_id, file_id, busid=102):
        """
        获取群文件资源链接
        """
        post_data = {
        ...
        return self._link("/get_group_file_url", post_data)
    ...
    def delete_msg(self, message_id):
        ...
        return self.add("/delete_msg", post_data)
    ...
    def upload_group_file(self, group_id, file, name, folder=None):
        ...
        return self.add("/upload_group_file", post_data)
...
class cqBot(cqSocket, cqEvent):
    ...
    def __init__(self, cqapi: cqHttpApi, host: str, group_id_list: list[int] = [], user_id_list: list[int] = [], options: dict[str, Union[list, str, int]] = {}):
        ...
        # 帮助信息模版
        self.help_text_format: str = "❤小心海来啦~❤\n{help_command_text}❤小心海在噢~❤"
        ...
        try:
            if plugin.rsplit(".", maxsplit=1)[0] == "pycqBot.plugin":
                ...
                plugin_obj = importlib.import_module(
                    f"pycqBot.plugin.{plugin}.{plugin}")
            else:
                plugin_obj = importlib.import_module(
            ...
            plugin_obj = eval(
                "plugin_obj.%s(self, self.cqapi, plugin_config[plugin])" % plugin)
            self.__plugin_list.append(plugin_obj)

            logging.info("%s 插件加载完成" % plugin)
        ...
        while True:
            ...
            for group_id in self.__group_id_list:
                ...
                try:
                    ...
                    self._plugin_event_run(
                        "timing_job_end", job, run_count, group_id)

                except Exception as err:
                    self.runTimingError(job, run_count, err, group_id)
                    self._plugin_event_run(
                        "runTimingError", job, run_count, err, group_id)
    ...
    def timing(self, function: Callable[[int], None], timing_name: str, options: dict[str, Any] = None) -> "cqBot":
        ...
        thread = Thread(target=self._timing_job, args=(
            self._timingList[timing_name],), name=timing_name)
        thread.setDaemon(True)
        thread.start()
    ...
    def get_bot_qqid(self):
        return self.__bot_qq
    ...
    def _meta_event_connect(self, message: dict[str, Any]) -> None:
        ...
        if self.messageSql is True:
            self.cqapi._create_sql_link(
                self.messageSqlPath, self.messageSqlClearTime)
    ...
    def runTimingError(self, job, run_count, err, group_id):
        ...
        定时任务执行错误
        """
        logging.error("定时任务 %s 在群 %s 执行错误... 共执行 %s 次 Error: %s" %
                      (job["name"], group_id, run_count, err))
        logging.exception(err)
    ...
    def _set_command_key(self, message: str):
        ...
        commandData: list = strToCqCode(message)
        commandSign = ''
        command = ''

        split: list = re.split('\s+', message)

        for i in range(len(split)):

            if self.commandSign in split[i]:

                commandSign = self.commandSign
                command = split[i][1::]
                if i + 1 <= len(split):
                    commandData += split[i + 1::]
    ...
    def _run_command(self, message, command_type):
        ...
        thread = Thread(target=run_command, args=(
            message, command_type, ), name="command")
        thread.setDaemon(True)
        thread.start()
    ...
    def check_command(self, message, from_id):
        ...
        """
        print("%s 使用指令: %s" %
              (self.user_log_srt(message), message.text))
        ...
        for cqCode in message.code:
            if cqCode["type"] == "at":
                if cqCode["data"]["qq"] == str(self.__bot_qq):
                    ...
                    self._plugin_event_run(
                        "at_bot", message, message.code, cqCode)
                    continue
    ...
    def notCommandError(self, message, from_id):
        ...
        self._bot_message_log(
            "指令 %s 不存在，请输入\"#help\"召唤小心海~" % message.text, message)
    ...
    def userPurviewError(self, message, from_id):
        ...
        指令用户组权限不足时错误
        """
        self._bot_message_log("%s 用户组权限不足... 指令 %s" % (
            self.user_log_srt(message), message.text), message)

    def purviewError(self, message, from_id):
        """
        指令权限不足时错误 (bot admin)
        """
        self._bot_message_log("%s 权限不足... 指令 %s" % (
            self.user_log_srt(message), message.text), message)
    ...
    def runCommandError(self, message, err):
        ...
        指令运行时错误
        """
        self._bot_message_log("指令 %s 运行时错误... Error: %s" %
    ...
    def notice_group_decrease_kick_me(self, message):
        ...
        async def _notice_group_decrease_kick_me(message):
            ...
            logging.info("bot 被 %s (qq=%s) T出群 685735591" % (
                user_data["data"]["nickname"], user_data["data"]["user_id"]))
